...
(function () {
  ...
  function initWechatQr() {
    ...
    var tipFallback = document.getElementById('qrTipFallback');
    if (!img || !placeholder) return;
    var tries = ['assets/wechat-qr.png', 'assets/wechat-qr.jpg'];
    var idx = 0;
    var tryNext = function () {
      if (idx >= tries.length) {
        img.style.display = 'none';
        if (fallback) fallback.style.display = '';
        if (tipFallback) tipFallback.style.display = '';
        return;
      }
      img.onerror = function () { idx++; tryNext(); };
      img.onload = function () { placeholder.classList.add('qr-placeholder--loaded'); };
      img.src = tries[idx];
    };
    tryNext();
  }
